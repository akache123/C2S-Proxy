...
void write_to_logfile(int response_size, string client_ip, string request_line, int status_code) {
    ...
    // Open the access log file in append mode
    int log_fd = open(args.log_file.first.c_str(), O_WRONLY | O_CREAT | O_APPEND, 0644);
    if (log_fd == -1) {
        cerr << "Error opening/accessing the access log file" << endl;
    ...
    char output[2000];
    int bytes_written = snprintf(output, sizeof(output), "%s %s \"%s\" %d %d\n", buffer, client_ip.c_str(), request_line.c_str(), status_code, response_size);
    if (bytes_written < 0) {
        cerr << "Error formatting the log message" << endl;
        close(log_fd);
        pthread_mutex_unlock(&log_mutex);
        return;
    }
    ...
    ssize_t write_result = write(log_fd, output, bytes_written);
    if (write_result == -1 || write_result < bytes_written) {
        cerr << "Error writing to the access log file" << endl;
    }
...
void get_client_request(client_request_info info, int version_num) {
    ...
    cout << "Received request from user" << endl;
...
int listen_for_requests(){
    ...
    cout << "Listen Port (before htons): " << args.listen_port << endl;
    unsigned short listen_port_network_byte_order = htons(args.listen_port);
    cout << "Listen Port (after htons): " << listen_port_network_byte_order << endl;
	...
	cerr << "Attempting to bind server socket..." << endl;
	...
	cerr << "Server socket bound successfully." << endl;
...
int main(int argc, char** argv) {
	...
	// Log the port number the server is attempting to bind to
	cerr << "Attempting to bind to port: " << args.listen_port << endl;
	
    //cout << "Listen Port: " << args.listen_port << endl;
    cout << "Forbidden Sites File Path: " << args.forbidden_sites_file.first << endl;
    cout << "Access Log File Path: " << args.log_file.first << endl;

    int server_socket = listen_for_requests();

    // Log when the server socket is closed
    cerr << "Proxy server socket closed." << endl;
